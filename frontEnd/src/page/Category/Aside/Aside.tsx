...
import { FilterByParams, initialFilterByParamsValues } from '../Category';
...
export default function Aside({ getFiltersValue }: Asidprops) {
  ...
  console.log("allPoroduct =>", Array.from(new Set(productContex.map(item => item.category))));
  ...
  useEffect(() => {
    getAllSize();
    setCategories(Array.from(new Set(productContex.map(item => item.category))))
    // setParamsValue(categoryName ? categoryName : initialFilterByParamsValues)
  }, [])
  ...
  const categoryNameFilterChangeHandler = (title: string) => {
    const existCategory = categoryFilterList.some(item => item === title);
    const newShoeSizeFilter = existCategory ? categoryFilterList.filter(item => item !== title) : [...categoryFilterList, title];
    const newFiltersValue: FilterVlueViewModel = {
      ...filtersValue,
      categoryFilterList: newShoeSizeFilter
    };
